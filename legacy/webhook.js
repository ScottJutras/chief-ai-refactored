...
const { sendMessage, sendQuickReply, sendTemplateMessage } = require('../services/twilio');
...
const { getAuthorizedClient } = require('../services/googleSheets');
...
async function updateBillInSheets(userId, billData) {
  try {
    ...
    const rowIndex = billRowIndex + 2;
    const existingRow = dataRows[billRowIndex];
    const updatedRow = [
      billData.date || existingRow[0],
      billData.billName,
      billData.amount || existingRow[2],
      billData.recurrence || existingRow[3],
      existingRow[4],
      'bill',
      existingRow[6],
      existingRow[7] || '',
      existingRow[8] || 'Unknown'
    ];
...
// Onboarding and Template Configurations
...
const confirmationTemplates = {
  ...
  deleteConfirmation: "HXabcdef1234567890abcdef123456789",
  teamMemberInvite: "HX1234567890abcdef1234567890abcdef",
...
// Retry logic for getUserProfile
...
router.post('/', async (req, res) => {
  console.log(`[DEBUG] POST / invoked at ${new Date().toISOString()}`);
  ...
  console.log(`[DEBUG] POST / received: From=${from || 'unknown'}, Body=${body || 'none'}, Media=${mediaUrl || 'none'}, MediaType=${mediaType || 'none'}, Headers=${JSON.stringify(req.headers)}`);
...
const contractorName = userProfileData.name || 'Your Company Name';           
  ...
  // user is already fully onboarded—send a simple TwiML greeting
  await db.collection('locks').doc(lockKey).delete();
  console.log(`[LOCK] Released lock for ${from} (already onboarded)`);
  const reply = `Hi ${userProfileData.name || 'there'}! How can I assist you today?`;
...
router.post('/parse', async (req, res) => {
  ...
  try {
    ...
    console.log(`[DEBUG] POST /parse done processing`);
    res.json({ data, reply, confirmed });
  } catch (error) {
    console.error(`[ERROR] in POST /parse:`, error.message);
...
router.post('/deep-dive', async (req, res) => {
  const { userId, tier = 'BASIC', file } = req.body;
  ...
  try {
    ...
    if (!userProfile.subscriptionTier) {
      ...
      await updateUserTokenUsage(userId, { messages: 1, aiCalls: 1 });
...
router.get('/', (req, res) => {
  console.log("[DEBUG] GET request received at /");
