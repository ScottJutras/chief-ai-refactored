...
router.post('/', lockMiddleware, userProfileMiddleware, tokenMiddleware, async (req, res, next) => {
  ...
  try {
    ...
    if (input.toLowerCase().includes('punch') || input.toLowerCase().includes('break') || input.toLowerCase().includes('lunch') || input.toLowerCase().includes('drive') || input.toLowerCase().includes('hours')) {
