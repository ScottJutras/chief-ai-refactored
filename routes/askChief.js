...
function hasDashboardToken(req) {
  ...
  const looksLikeDashboardBearer = /bearer\s+.+/i.test(String(auth));

  // If you want: treat any bearer as potentially dashboard token
  // But we will prefer cookie-based for dashboards.
  return hasCookie || looksLikeDashboardBearer;
}
...
// Rule:
// - If tenant.owner_id digits is missing => treat as NOT_LINKED (they must link WhatsApp / set tenant owner).
// - Else fetch plan from public.users where user_id = ownerDigits.
// - Decide paid via: stripe_subscription_id OR subscription_tier/plan_key OR active trial_end.
const REQUIRE_PLAN = String(process.env.ASK_CHIEF_REQUIRE_PLAN || "1") === "1"; // default ON

if (REQUIRE_PLAN) {
  try {
    // We only have tenant in portal mode (Supabase bearer).
    // In dashboard mode, you can choose to allow for now.
    if (!req.tenantId) {
      ...
      console.info("[ASK_CHIEF_OWNER_DIGITS]", { tenantId: req.tenantId, ownerDigits: ownerDigits || null });
      ...
      if (!url || !serviceKey) {
        ...
        return res.status(500).json({ ok: false, code: "ERROR", message: "Server misconfigured (supabase)." });
      }
      ...
      // This usually means WhatsApp/owner setup hasn't completed for this tenant.
      if (!userRow?.user_id) {
        return res.status(200).json({
          ok: false,
          code: "NOT_LINKED",
      ...
      // Optional: if you ever store current_period_end, treat it as active if in future
    ...
    // ---------------- Execute Brain ----------------
    const actorKey =
      String(req.portalUserId || "").trim() || ownerId || "portal";
    ...
    const out = await answerChief({
      ...
      context: { tenantId, ownerDigits: ownerId || null, portalRole: req.portalRole || null }
    });
