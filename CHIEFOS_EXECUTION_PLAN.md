...
- [ ] **SignQuote handler (NEXT SESSION).** Third new-idiom handler. Consumes `sent` quote + share_token + signer identity + signature PNG → creates `chiefos_quote_signatures` row (Migration 4) with server-computed SHA-256 hash of canonical version per §4 → locks the version (`locked_at` = NOW(), `server_hash` populated) → emits `lifecycle.signed` + `lifecycle.locked` + `notification.sent` (contractor confirmation email). Soft step-up via name-match per §14.12 — mismatch emits `integrity.name_mismatch_signed`. Two candidate principles flagged from SendQuote (post-commit paired notification events; `correlation_id` on notification.* linking to lifecycle.*) get their second exercise here; if they hold, formalize at session close as §17.19 + §14 expansion.
