...
async function getOpenShift(owner_id, user_id){
  const { rows } = await pg.query(
    `SELECT * FROM time_entries
      ...
      LIMIT 1`, [owner_id, user_id]);
  return rows[0] || null;
}
async function ensureNoOverlapChild(owner_id, parent_id, kind){
  await pg.query(
    `UPDATE time_entries SET end_at_utc=now()
...
async function insertEntry(row){
  ...
  const { rows } = await pg.query(`INSERT INTO time_entries (${cols.join(',')}) VALUES (${vals}) RETURNING *`, params);
  return rows[0];
}
async function closeEntryById(owner_id, id){
  const { rows } = await pg.query(
    `UPDATE time_entries
      ...
      WHERE owner_id=$1 AND id=$2 AND end_at_utc IS NULL
      RETURNING *`, [owner_id, id]);
  return rows[0] || null;
}
async function fetchPolicy(owner_id){
  const { rows } = await pg.query(`SELECT * FROM employer_policies WHERE owner_id=$1`, [owner_id]);
...
async function touchKPI(owner_id, job_id, day){
  await pg.query(`INSERT INTO kpi_touches (owner_id, job_id, day) VALUES ($1,$2,$3)`, [owner_id, job_id, day]);
}
...
function extractSegmentNarrative(text){
  ...
  const re=new RegExp(
    `^([\\p{L}\\p{M}.'-]+(?:\\s+[\\p{L}\\p{M}.'-]+){0,2})\\s+(?:forgot|did\\s*not|didn't|needs?|need)\\s+to\\s+(start|stop|end)\\s+(?:his|her|their|the)?\\s*(?:${ordinal}\\s*)?(break|drive)\\b`,'iu');
...
async function handleClock(ctx, cil){
  ...
  const { owner_id, user_id, job_id, created_by } = ctx;

  if (parsed.action === 'in'){
    const open = await getOpenShift(owner_id, user_id);
    if (open) return { text: `You’re already clocked in since ${open.start_at_utc}.` };
    await insertEntry({ owner_id, user_id, job_id, parent_id:null, kind:'shift', start_at_utc:at, created_by, meta:{} });
...
Tip: add @ Job Name for context (e.g., “clock in @ Roof Repair”).`);
    ...
    if (segNarr){
      ...
      if (segNarr.seg==='break'){ if (segNarr.action==='start'){ isBreakStart=true; isBreakStop=false; } else { isBreakStop=true; isBreakStart=false; } }
      else if (segNarr.seg==='drive'){ if (segNarr.action==='start'){ isDriveStart=true; isDriveStop=false; } else { isDriveStop=true; isDriveStart=false; } }
    }
    ...
    if (tsOverride){
      ...
      if (diffMin > 2 && resolvedType){
        await pg.savePendingAction({
          ownerId: String(ownerId).replace(/\D/g,''), userId: from,
          kind: 'backfill_time',
          payload: { target, type: resolvedType, tsOverride, jobName }
        });
    ...
    if (mForceIn){
      ...
      await pg.logTimeEntryWithJob(ownerId, forced, 'clock_in', tsOverride || now, jobName, tz, { requester_id: from });
      return twiml(res, `✅ Forced clock-in recorded for ${forced} at ${formatLocal(tsOverride || now, tz)}.`);
    }
    if (mForceOut){
      const forced = mForceOut[1].trim();
      await pg.logTimeEntryWithJob(ownerId, forced, 'clock_out', tsOverride || now, jobName, tz, { requester_id: from });
    ...
    if (isClockIn2){
      ...
      await pg.logTimeEntryWithJob(ownerId, target, 'clock_in', tsOverride || now, jobName, tz, { requester_id: from });
    ...
    if (isClockOut2){
      ...
      await pg.logTimeEntryWithJob(ownerId, target, 'clock_out', tsOverride || now, jobName, tz, { requester_id: from });
    ...
    if (isBreakStart2){
      ...
      await pg.logTimeEntryWithJob(ownerId, target, 'break_start', tsOverride || now, jobName, tz, { requester_id: from });
    ...
    if (isBreakStop2){
      ...
      await pg.logTimeEntryWithJob(ownerId, target, 'break_stop', tsOverride || now, jobName, tz, { requester_id: from });
    ...
    if (isDriveStart2){
      ...
      await pg.logTimeEntryWithJob(ownerId, target, 'drive_start', tsOverride || now, jobName, tz, { requester_id: from });
    ...
    if (isDriveStop2){
      ...
      await pg.logTimeEntryWithJob(ownerId, target, 'drive_stop', tsOverride || now, jobName, tz, { requester_id: from });
