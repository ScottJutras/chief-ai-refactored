...
- [x] **VoidQuote handler (COMPLETE 2026-04-25).** Sixth new-idiom CIL handler. Header-only state-driven transition per §3A (any of five prior states → voided; version row passes through unchanged). State-machine idempotent (already-voided returns `_avoidShape` without DB write). Router-registered in `src/cil/router.js:39`. §31 ceremony validated via `scripts/real_void_quote_ceremony.js`. Session 4 §3 close-out commit `f4b54fe4`.
- [x] **ReissueQuote handler (COMPLETE 2026-04-25).** Seventh and final Phase A CIL handler. Closes Phase A. Voided → new draft version (INSERT new chiefos_quote_versions row; copy line items via INSERT...SELECT; swing chiefos_quotes.current_version_id; clear voided_at/voided_reason per chiefos_quotes_voided_consistency CHECK). §17.8 entity-table dedup via new partial UNIQUE on `chiefos_quote_versions(owner_id, source_msg_id)` (Migration 2026_04_25). Constitutional immutability extended to block UPDATE/DELETE on superseded versions (header-pointer divergence detection). §17.26 reservation outcome: not needed — implementation slots into §17.8 + §3A + the immutability extension. 16 unit tests pass; 9 integration tests authored (cross-tenant isolation + idempotency replay BLOCKING tests included). §32 ceremony script + decisions-log entry landed.
- [x] CIL types complete in quote spine: CreateQuote, SendQuote, SignQuote, ViewQuote, LockQuote, VoidQuote, ReissueQuote *(all extend `src/cil/schema.js::BaseCILZ` per §17.1; CIL-retry dedup via `(owner_id, source_msg_id)` UNIQUE per §17.8–§17.11; return shape family-wide per §17.15)*. **Phase A closed 2026-04-25.**
