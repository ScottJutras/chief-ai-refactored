...
- [ ] **Migration 5 (next session — 2026-04-20).** Restructure `chiefos_tenant_counters` to support per-tenant per-kind counters per §17.13; add generic `allocateNextDocCounter(tenantId, counterKind, client)` function. Design shape committed as §18; SQL proposal + discovery (backfill strategy, in-flight allocation safety, PK transition mechanics, function update approach) + apply + verify in a dedicated session. Matches the per-session cadence established by Migrations 1–4. *(see `docs/QUOTES_SPINE_DECISIONS.md` §18)*
- [ ] CIL types in `src/cil/quotes.js`: CreateQuote (opens session after Migration 5 — all design decisions locked per §20/§19/§17.15/§17.16/§17.17/§17.14/§17.12). Then SendQuote, SignQuote, LockQuote, VoidQuote, ReissueQuote *(all extend `src/cil/schema.js::BaseCILZ` per §17.1; CIL-retry dedup via `(owner_id, source_msg_id)` UNIQUE on root entities per §17.8–§17.11; return shape family-wide per §17.15)*
