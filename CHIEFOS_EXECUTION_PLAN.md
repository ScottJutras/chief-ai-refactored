...
- [ ] **SignQuote Phase 2B — Storage helper implementation (NEXT SESSION).** Build `src/cil/quoteSignatureStorage.js` with all exports per §25.7; comprehensive test suite `src/cil/quoteSignatureStorage.test.js` with cross-version regression lock; micro-migration adding `chiefos_qs_png_storage_key_format` DB CHECK per §25.3; local `.env` confirmation of `SUPABASE_URL`/`NEXT_PUBLIC_SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY`; manual Supabase dashboard bucket provisioning per §25.7 (`chiefos-signatures` private bucket, 2MB limit, image/png-only).
- [ ] **SignQuote Phase 2C — Storage pipeline ceremony (SESSION AFTER PHASE 2B).** Synthetic 100×100 PNG fixture upload to production `chiefos-signatures` bucket; retrieve via both `getSignatureForOwner` + `getSignatureViaShareToken` helpers; byte-identity verification (upload SHA == download SHA); proves pipeline end-to-end against production Supabase before Phase 3 SignQuote handler consumes it. `mission-logo.png` is NOT used — deferred to Extension 5 (tenant-profile table + portal UI + logo-rendering integration dependencies).
- [ ] **SignQuote Phase 3 — handler implementation + real-write ceremony (SESSION AFTER PHASE 2C).** Third new-idiom handler. Consumes `sent` quote + share_token + signer identity + signature PNG → validates share_token + state → computes `server_hash` via Phase 1's `computeVersionHash` → uploads PNG via Phase 2's `uploadSignaturePng` → inserts `lifecycle.signed` event FIRST (NOT NULL composite FK from signature) → creates `chiefos_quote_signatures` row → locks the version (`locked_at` = NOW(), `server_hash` populated; status + locked_at co-transition per `chiefos_qv_status_locked_consistency` CHECK) → emits `lifecycle.locked` + optional `notification.sent` (contractor confirmation email) → optional `integrity.name_mismatch_signed` on name-match failure per §11a. Two candidate principles flagged from SendQuote (post-commit paired notification events; `correlation_id` on notification.* linking to lifecycle.*) get their second exercise here; if they hold, formalize at session close as §17.19 + §14 expansion. First handler to exercise actor role outside `owner` (customer-initiated sign; validates §17.17 from a new angle).
- [ ] CIL types remaining after SignQuote: LockQuote, VoidQuote, ReissueQuote *(all extend `src/cil/schema.js::BaseCILZ` per §17.1; CIL-retry dedup via `(owner_id, source_msg_id)` UNIQUE on root entities per §17.8–§17.11; return shape family-wide per §17.15)*
