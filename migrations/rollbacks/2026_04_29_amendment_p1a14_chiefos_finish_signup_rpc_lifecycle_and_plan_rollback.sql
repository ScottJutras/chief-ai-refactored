-- Rollback for: 2026_04_29_amendment_p1a14_chiefos_finish_signup_rpc_lifecycle_and_plan.sql
--
-- Restores chiefos_finish_signup() to the P1A-13 body (re-adds the
-- public.users INSERT clause that writes plan_key='free').
--
-- Apply order:
--   1. This rollback FIRST (RPC must reference users.plan_key while the
--      column exists).
--   2. THEN the schema rollback (drops chiefos_tenants lifecycle/plan and
--      restores users.plan_key with default 'free').
--
-- Inverse ordering will fail because:
--   - If schema rollback runs first, users.plan_key exists but P1A-14 RPC
--     does not write to it; new tenants would have NULL plan_key and the
--     restored CHECK ('free','starter',...) would still pass since column
--     default 'free' applies — actually safe, but RPC body is the wrong
--     shape. Apply this rollback to restore the canonical P1A-13 body.
--
...
BEGIN
  ...
  -- P1A-13 shape: writes plan_key='free' to public.users
