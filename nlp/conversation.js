...
async function converseAndRoute(userText, { userProfile, ownerId, convoState, memory } = {}) {
  ...
  // ⬇️⬇️⬇️ FAST-PATH FOR TASKY UTTERANCES ⬇️⬇️⬇️
  if (looksLikeTask(userText)) {
    try {
      const parsed = parseTaskUtterance(userText, { tz: 'America/Toronto' });
      // Don’t create the task here—just normalize & pass args to the tasks handler.
      return {
        handled: false,
        route: 'tasks',
        ...
        args: {
          title: parsed.title,
          dueAt: parsed.dueAt,
          assigneeName: parsed.assignee || null, // handler resolves name -> user_id
        }
      };
    } catch (e) {
