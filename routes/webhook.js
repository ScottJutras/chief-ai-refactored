...
// === END REMINDERS-FIRST SHORT-CIRCUIT ===
  ...
  if (!isTasky && isTextOnly && pendingState?.pendingMedia) {
    ...
    // Expense / Revenue / Time entry confirmation (UNCHANGED)
...
// ⬆️ END pending text-reply handler
...
try {
  ...
  try {
    ...
    if (routed) {
      ...
      // Fast-path to concrete handlers when the router asks for them.
      // We keep later sections intact, but if we successfully handle here we return early.
      const route = routed.route;
      if (!routed.handled && route) {
        let handled = false;
        let responseText = '';

        // Pass parsed args along (e.g., title / assignee / dueAt for tasks, etc.)
        res.locals = res.locals || {};
        res.locals.intentArgs = routed.args || null;

        if (route === 'tasks') {
          // e.g., normalized: "task - Email quote"
          handled = await tasksHandler(
            from,
            routed.normalized,
            userProfile,
            ownerId,
          ...
          if (handled !== false) {
            await logEvent(tenantId, userId, 'tasks.create', {
          ...
          if (typeof expenseFn === 'function') {
            handled = await expenseFn(
              from,
              routed.normalized, // keep normalized text for your expense parser
              userProfile,
              ownerId,
              ownerProfile,
              isOwner,
            ...
            if (handled !== false) {
              await logEvent(tenantId, userId, 'expense.add', {
                normalized: routed.normalized,
                args: routed.args || {}
              });
              // Examples of lightweight learning from args (optional):
              if (routed.args?.alias && (routed.args.vendor || routed.args.job)) {
                await upsertMemory(tenantId, userId,
                  `alias.vendor.${routed.args.alias.toLowerCase()}`,
                  { name: routed.args.vendor || routed.args.job }
                );
              }
          ...
          // Ensure normalization consistent with your timeclock handler
          const normalized = normalizeTimeclockInput(routed.normalized, userProfile);
          handled = await handleTimeclock(
            from,
            normalized,
            userProfile,
            ownerId,
          ...
          if (handled !== false) {
            await logEvent(tenantId, userId, 'timeclock', {
              normalized,
              args: routed.args || {}
            });
            // Keep active job if supplied by router
          ...
          if (typeof jobFn === 'function') {
            handled = await jobFn(
              from,
              routed.normalized, // e.g., "create job Roof Repair"
              userProfile,
              ownerId,
              ownerProfile,
              isOwner,
            ...
            if (handled !== false) {
              await logEvent(tenantId, userId, 'job.create', {
                normalized: routed.normalized,
                args: routed.args || {}
              });
            }
        ...
        if (handled) {
          await saveConvoState(tenantId, userId, {
            ...
            history: [...(convo.history || []).slice(-4), {
              ...
              intent: routed.intent || route || null
            }]
          });
          if (!res.headersSent) ensureReply(res, responseText);
          return; // ✅ We handled it here; stop the pipeline
        }
        // else: fall through to the rest of your pipeline
      }
    }
  } catch (e) {
  ...
  function pickFirstMedia(reqBody = {}) {
    // Twilio sends NumMedia and 0-indexed keys: MediaUrl0, MediaContentType0, etc.
    const n = parseInt(reqBody.NumMedia || '0', 10) || 0;
    if (n <= 0) return { mediaUrl: null, mediaType: null, num: 0 };

    // Prefer 0th item (WhatsApp voice notes are usually a single file)
  ...
  // If you already computed mediaUrl/mediaType earlier, override them here with the robust ones:
  if (!mediaUrl && picked.mediaUrl)   mediaUrl  = picked.mediaUrl;   // keep your existing variable names
  if (!mediaType && picked.mediaType) mediaType = picked.mediaType;

  // Trace what came in from Twilio so we can debug voice notes
...
try {
  ...
  try {
    ...
    // Helper to pluck plain text from TwiML for logging
