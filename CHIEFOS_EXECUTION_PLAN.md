...
- [ ] **CreateQuote handler (NEXT SESSION).** Write `src/cil/quotes.js` with `handleCreateQuote` + `CreateQuoteCILZ` + `TenantSnapshotZ` + `CustomerSnapshotZ`. All architectural decisions locked: §20 (input contract), §19/§17.16 (plan gating + `gateNewIdiomHandler` + `canCreateQuote`), §17.15 (return shape `{ok:true, quote:{...}, meta:{...}}`), §17.17 (actor owner-only), §17.14 (NULL-then-UPDATE INSERT sequence), §17.12 (frozen handler map registration), §17.13 (per-tenant `quote` counter via `allocateNextDocCounter`), §17.10 (`classifyUniqueViolation` on `chiefos_quotes_source_msg_unique`). Register in `NEW_IDIOM_HANDLERS` in `src/cil/router.js`. Jest unit tests for: schema validation, plan gate (allow + deny-requires + deny-capacity), actor gate (owner-only), happy path (creates header + v1 + line items + audit events), idempotent retry (source_msg_id match → returns prior quote with `meta.already_existed: true`).
- [ ] CIL types remaining in `src/cil/quotes.js` after CreateQuote: SendQuote, SignQuote, LockQuote, VoidQuote, ReissueQuote *(all extend `src/cil/schema.js::BaseCILZ` per §17.1; CIL-retry dedup via `(owner_id, source_msg_id)` UNIQUE on root entities per §17.8–§17.11; return shape family-wide per §17.15)*
