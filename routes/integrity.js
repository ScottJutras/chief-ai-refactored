...
//
// services/integrity.js was realigned with the chiefos_integrity_chain_stamp
// trigger field-set in commit (cutover-integration-parity Bundle 4); the
// 503 gate that was on /verify and /record/:id has been removed and the
// real handlers restored. Field-set drift is locked by
// __tests__/integrity.fieldsets.test.js + services/integrity.fixtures.js.
  ...
  try {
    const { rows } = await pg.queryWithTimeout(
      `SELECT id, record_hash, previous_hash, hash_version, hash_input_snapshot,
              tenant_id, owner_id, kind, amount_cents, currency, date,
              source, source_msg_id, created_at
