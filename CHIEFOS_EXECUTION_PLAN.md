...
- [ ] **Next session candidates (flagged 2026-04-21; no decision required).** Five options available — pick any:
    1. *SendQuote correlation_id backfill* — hygiene; pass correlationId through SendQuote's `emitLifecycleSent` / notification helpers so §17.21 asymmetry closes. ~15 min; low risk.
    2. *ReissueQuote handler* — triggers §17.20 again; formalizes superseded-token machinery via populating `superseded_by_version_id`. Medium-size handler session.
    3. *LockQuote handler* — signed → locked cosmetic transition. Small handler.
    4. *VoidQuote handler* — draft/sent/viewed → voided (terminal). Small-medium; defines voided semantics.
    5. *Different handler family entirely* — expenses, revenue capture, timeclock, or other ChiefOS domain.
