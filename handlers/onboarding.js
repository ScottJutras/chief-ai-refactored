...
const { saveUserProfile, createSpreadsheetForUser, sendSpreadsheetEmail, generateOTP } = require('../services/postgres');
...
async function handleOnboarding(from, input, userProfile, ownerId, res) {
  ...
  try {
    ...
    if (!state || (!userProfile.onboarding_in_progress && responseMsg.toLowerCase() === 'start onboarding')) {
      ...
      state = {
        ...
        detectedLocation: {
          ...
          province: userProfile.province || 'Ontario'
        }
      };
      ...
      reply = "Welcome to Chief, your personal CFO! Let’s set up your account to start growing your business.\nPlease reply with your full name.";
      await releaseLock(lockKey);
      return res.send(`<Response><Message>${reply}</Message></Response>`);
    }
    ...
    if (state.step === 0) {
      ...
      await sendTemplateMessage(
        ...
        [
          { type: "text", text: userProfile.province || "Ontario" },
          { type: "text", text: userProfile.country || "Canada" }
        ]
      );
      await releaseLock(lockKey);
      ...
      if (lcResponse === "yes") {
        ...
        await db.runTransaction(async (transaction) => {
          ...
          transaction.update(userRef, {
            ...
            spreadsheet_id: admin.firestore.FieldValue.delete()
          });
        });
      ...
      if (!isValidProvince || !isValidCountry) {
        ...
        if (!isValidProvince || !isValidCountry) {
          ...
          if (state.invalidLocationAttempts > 3) {
            throw new Error("Too many invalid location attempts");
          }
          reply = "Unable to parse location. Please use 'State/Province, Country' format (e.g., 'Ontario, Canada').";
          await releaseLock(lockKey);
          return res.send(`<Response><Message>${reply}</Message></Response>`);
        }
      }
      ...
      if (lcResponse === 'yes') {
        ...
        await setPendingTransactionState(from, state);
        reply = "Please share your email address to receive your financial dashboard.";
        await releaseLock(lockKey);
        return res.send(`<Response><Message>${reply}</Message></Response>`);
      ...
      try {
        ...
        userProfile.spreadsheetId = spreadsheetId;
        userProfile.dashboard_token = (await getUserProfile(from)).dashboard_token;
        await saveUserProfile(userProfile);
        await sendSpreadsheetEmail(responseMsg, spreadsheetId);
        const dashboardUrl = `https://chief-ai-<your-team>.vercel.app/dashboard/${from}?token=${userProfile.dashboard_token}`;
        await sendTemplateMessage(
          from,
        ...
        throw new Error(`Failed to process dashboard or messages: ${error.message}`);
      }
      ...
      await saveUserProfile(userProfile);
      reply = `Got it, ${userProfile.name}! Industry set to ${responseMsg}. Keep logging—next up, I’ll ask your financial goal when you add a bill or revenue.`;
      await deletePendingTransactionState(from);
      await releaseLock(lockKey);
      return res.send(`<Response><Message>${reply}</Message></Response>`);
      ...
      if (!responseMsg.match(/\d+/) || (!responseMsg.includes('profit') && !responseMsg.includes('debt'))) {
        reply = "That doesn’t look like a goal. Try 'Grow profit by $10,000' or 'Pay off $5,000 debt'.";
        await releaseLock(lockKey);
        return res.send(`<Response><Message>${reply}</Message></Response>`);
      }
      ...
      reply = `Goal locked in: "${responseMsg}" (${currency} ${userProfile.goalProgress.target.toFixed(2)}). You’re unstoppable, ${userProfile.name}!`;
      await deletePendingTransactionState(from);
      await releaseLock(lockKey);
      return res.send(`<Response><Message>${reply}</Message></Response>`);
    } else {
      throw new Error("Unknown onboarding state");
    }
  } catch (err) {
    console.error(`Error in handleOnboarding: ${err.message}`);
    await releaseLock(lockKey);
    throw err;
  }
}
