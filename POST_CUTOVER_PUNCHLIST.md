...
7. **For every submodule pointer bump in a parent PR**, verify the referenced submodule SHA is reachable from the submodule's `main` branch — not just any branch on origin. If only reachable on a feature branch, ensure that submodule branch has its own PR opened + merged either before or in lockstep with the parent PR. Vercel resolves submodule SHAs from origin (by SHA), so the parent's build won't fail, BUT the submodule's own production deploy follows its `main` only — leaving production in a half-merged state where the parent serves the new code and the submodule still serves stale code. Surfaced 2026-04-28 (12th uncommitted-claimed-shipped instance, at branch-level layer) when chiefos-site/main was at gate state `a063416` despite chief-ai-refactored/main referencing chiefos-site SHA `a8e2b40` from the merged Path α + cleanup PRs.
