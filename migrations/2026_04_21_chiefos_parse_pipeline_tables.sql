...
--
-- Creation order: parse_jobs → vendor_aliases → parse_corrections (FK).
--
...
-- if future migrations need to reference parse_jobs with tenant+owner
-- co-validation. Cheap to have, expensive to retrofit. See Engineering
-- Constitution §2 (dual-boundary) and quotes-spine precedent.
...
COMMENT ON TABLE public.parse_corrections IS
  'Per-field correction log. Written when an owner edits a parsed field before confirming. Feeds vendor_aliases upsert for merchant corrections — the enrichment moat. FK to parse_jobs(id); tenant_id carries the portal boundary for RLS.';
