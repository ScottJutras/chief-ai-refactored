...
| 2026-04-29 | `2026_04_29_amendment_p1a10_legal_acceptances_service_role_update.sql` | P1A-10. `GRANT UPDATE ON chiefos_legal_acceptances TO service_role`. Original `rebuild_identity_tenancy.sql` granted only INSERT+SELECT to service_role; PostgreSQL requires UPDATE for `INSERT ... ON CONFLICT DO UPDATE` even when no row conflicts. Was 42501'ing at runtime after P1A-9 unblocked the parser. Restores **partial** parity with peer tables. **DELETE intentionally NOT granted** — see P1A-11 schema comment for the full rationale and the warning to future engineers not to "fix" this divergence as cleanup. | `REVOKE UPDATE` |
| 2026-04-29 | `2026_04_29_amendment_p1a11_legal_acceptances_asymmetry_comment.sql` | P1A-11. Schema-side `COMMENT ON TABLE chiefos_legal_acceptances` documenting the intentional service_role DELETE-grant asymmetry from P1A-10. Discoverable via `\d+` and any introspection tool, so the rationale survives outside the migration history. Pure documentation; no behavioral change. | `COMMENT ON TABLE ... IS NULL` |
| 2026-04-28 | `2026_04_28_amendment_p1a12_sequence_usage_grants.sql` | P1A-12. `GRANT USAGE, SELECT, UPDATE ON ALL SEQUENCES IN SCHEMA public TO service_role` + matching `ALTER DEFAULT PRIVILEGES`. Surfaced 2026-04-28 via /api/log preview testing — `time_entries_v2_id_seq` 42501'd because rebuild's identity-tenancy migration granted INSERT on tables but missed sequence-level grants. Inventory found 7 sequences with NO service_role grant (all 7 latent-broken pre-fix). Comprehensive fix covers all current + future sequences. | `REVOKE USAGE, SELECT, UPDATE` |
